...
#-------------------------------------------------------------------------------
# containers
b_urnings = matrix(0, ncol = 7, nrow = length(Student_urn_size) * length(Pi))
...
for(us in 1:length(Student_urn_size)){
  ...
  b_urnings[((us-1)*length(Pi) + 1):(us*length(Pi)),] = res$baseline
}

#-------------------------------------------------------------------------------
# saving the output (RDS)
#-------------------------------------------------------------------------------
saveRDS(b_urnings, "output/b_u1.RDS")
saveRDS(b_sys_urnings, "output/b_sys_u1.RDS")
